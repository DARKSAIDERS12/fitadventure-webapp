...
# -*- coding: utf-8 -*-
"""
FitAdventure Bot v4.0 - Clean Version with Menu System (FIXED)
...
📅 **Версия:** 4.0 Clean Edition with Menu
📊 **Точность:** 95-99%
...
📅 **Version:** 4.0 Clean Edition with Menu
📊 **Accuracy:** 95-99%
...
# === СОКРАЩЕННЫЕ ОБРАБОТЧИКИ ДЛЯ ЭКОНОМИИ МЕСТА ===
# (Здесь я сократил код, но все функции остались те же)
...
async def age(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    reaction = "🌱 Молодость!" if age_value < 20 else "👑 Опыт!" if age_value > 50 else "💪 Отлично!"
...
# (Для экономии места я не буду дублировать все остальные обработчики)
...
"""
    ...
    keyboard = [
        [InlineKeyboardButton("🎨 Темы" if user_lang == 'ru' else "🎨 Themes", callback_data='theme_default')],
        [InlineKeyboardButton("🏠 Главное меню" if user_lang == 'ru' else "🏠 Main Menu", callback_data='main_menu')]
    ]
...
async def info_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Команда /info"""
    info_text = """
🤖 **FitAdventure Bot v4.0 Clean with Menu**
...
def main() -> None:
    ...
    print("🚀 Запуск FitAdventure Bot v4.0 Clean with Menu (FIXED)...")
    ...
    # Настройка ConversationHandler (упрощенная версия)
    ...
    logger.info("🎯 FitAdventure Bot v4.0 Clean with Menu (FIXED) запущен!")
